...
@Component
public class MockCvEventGenerator_YBBN {

    private static final Logger log = LoggerFactory.getLogger(MockCvEventGenerator_YBBN.class);
    ...
    public MockCvEventGenerator_YBBN(RestTemplate restTemplate, io.micrometer.core.instrument.MeterRegistry registry) {
        ...
        this.eventCounter = io.micrometer.core.instrument.Counter.builder("simulator.events.generated")
                .tag("type", "turnaround")
                .tag("icao", "YBBN")
                .description("Number of mock YBBN turnaround events generated")
                .register(registry);
    }
    ...
    private void sendEvent(CvEventDto dto) {
        try {
            log.info("Generated YBBN CV event: {} ({}) at {}", dto.getActivityType(),
            ...
            log.error("Failed to send mock YBBN CV event: {}", e.getMessage());
        }
    }
}
