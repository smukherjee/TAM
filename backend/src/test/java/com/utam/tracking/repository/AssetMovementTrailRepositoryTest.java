...
class AssetMovementTrailRepositoryTest {
        ...
        void findByAssetIdAndTimestampBetween_ReturnsOrderedTrail() {
                ...
                List<AssetMovementTrail> result = repository
                                .findByAssetIdAndTenantCodeAndTimestampBetweenOrderByTimestampAsc(
        ...
        void findByAssetIdentifierAndTimestampBetween_ReturnsTrailByIdentifier() {
                ...
                when(repository.findByAssetIdentifierAndTenantCodeAndTimestampBetweenOrderByTimestampAsc(
                                eq(assetIdentifier), eq(tenantCode), any(ZonedDateTime.class),
                                any(ZonedDateTime.class)))
                                .thenReturn(Arrays.asList(trail1, trail2, trail3));

                List<AssetMovementTrail> result = repository
                                .findByAssetIdentifierAndTenantCodeAndTimestampBetweenOrderByTimestampAsc(
        ...
        void findByZoneAndTimeRange_ReturnsZoneMovements() {
                ...
                when(repository.findByZoneAndTimeRange(eq(zoneId), eq(tenantCode), any(ZonedDateTime.class),
                                any(ZonedDateTime.class)))
