...
import { DiscrepancyFilters, DISCREPANCY_TYPE_LABELS } from '../../types/tracking';
import { 
    Filter, 
    X, 
    Calendar,
...
interface DiscrepancyFiltersPanelProps {
    filters: DiscrepancyFilters;
    onFilterChange: (filters: Partial<DiscrepancyFilters>) => void;
    tenantCode: string;
}

/**
 * DiscrepancyFiltersPanel - Filter controls for movement discrepancies
 * Feature: 005-asset-tracking-security
 * Phase 10: Movement Discrepancy Report
 */
const DiscrepancyFiltersPanel: React.FC<DiscrepancyFiltersPanelProps> = ({
    filters,
    onFilterChange,
    tenantCode
}) => {
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-2">
                    <input
                        ...
                        className="block w-full px-3 py-2 border border-gray-600 rounded-md 
                            text-sm bg-gray-700 text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                    <input
                        type="date"
                        value={endDate}
            ...
            <div className="space-y-2">
                ...
                <select
                    ...
                    onChange={(e) => onFilterChange({ discrepancyType: e.target.value as any || undefined })}
                    className="block w-full px-3 py-2 border border-gray-600 rounded-md 
                        text-sm bg-gray-700 text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
                    <option value="">All Types</option>
                    {Object.entries(DISCREPANCY_TYPE_LABELS).map(([value, label]) => (
                ...
                <div className="space-y-2">
                    <button
                        onClick={() => onFilterChange({ 
                            acknowledged: false, 
                            discrepancyType: 'UNEXPECTED_MOVEMENT' 
                        })}
                    ...
                    <button
                        onClick={() => onFilterChange({ 
                            acknowledged: false,
                            discrepancyType: 'SPEED_ANOMALY'
                        })}
                        className="w-full px-3 py-2 text-left text-sm text-purple-400 
                            bg-purple-900/50 rounded-md hover:bg-purple-900/70 border border-purple-700"
                    >
                        ⚡ Speed Anomalies
                    </button>
                    <button
...
export default DiscrepancyFiltersPanel;
